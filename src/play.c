...
void audio_callback(ma_device *pDevice, void *pOutput, const void *pInput, ma_uint32 framecount) {
  ...
  for (int i = 0; i < framecount; i++) {
    if (synth->is_active) {
      double sample = oscillator_next(&synth->osc);
      out[i] = (float)(sample * 0.3);
    } else {
      out[i] = 0.0;
    }
  }
...
void play() {
  ...
  SynthState synth = {0};
  synth.current_wave = (WaveType)wave;
  synth.is_active = false;
  oscillator_init(&synth.osc, synth.current_wave, 440.0);
  ...
  while (running) {
    ...
    if (key != 0) {  // A key was pressed
      if (key == 'q') {
        ...
        if (key != last_key) {
          // dont recalculate diff if not needed.
          double freq = note_to_freq(key);
          oscillator_init(&synth.osc, synth.current_wave, freq);
          last_key = key;
        }
